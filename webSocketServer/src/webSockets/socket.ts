...
export const startSocket = (server: HttpServer) => {
  ...
  io.on("connection", async (socket) => {
    ...
    if (!interviewDetail) {
       new ApiError(404, "interview details not found")
    ...
    const dgSocket = await createDeepgramConnection(
      async (transcript: string) => { 
        try {
          ...
          const numberOfQuestions = (interviewDetail as any)?.numberOfQuestions ?? 5;
          ...
          const sentences = await response.sendMessage(transcript, (sentence: string) => {
            try {
              ...
              interviewId: interviewDetail?.id,
              message: "error while sending response to model please refresh the page", 
              ttsConnectionError: error.message
          ...
          if (sentences!.parsed.action === "CANT_CONTINUE") {
            socket.emit("error-message", {
              interviewId: interviewDetail?.id,
              message: "cant continue interview due to inappropriate behaviour"
    ...
    socket.on("audio-chunk", (chunk) => {
      try { 
        ...
        console.log("Audio send error:", error.message);
        socket.emit("error-message", {
              interviewId: interviewDetail?.id,
    ...
    socket.on("disconnect", () => {
      ...
      try {
        dgSocket.sendCloseStream({ 
         ...
         socket.emit("error-message", {
              interviewId: interviewDetail?.id,
              message: "dg connection is being closed"
      ...
      try {
       ...
       socket.emit("error-message", {
              interviewId: interviewDetail?.id,
              message: "connection is being closed"
